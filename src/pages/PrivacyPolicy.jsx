...
const PrivacyPolicy = () => {
  ...
  useEffect(() => {
    const getTerms = async () => {
      try {
        const res = await commonCmsService("privacy-policy");
        // console.log(res?.data?.data[0].content)
        setPrivacy(res?.data?.data[0].content || [])
      } catch (err) {
        console.log(err)
      }
    }
    ...
    <section className="w-full bg-white overflow-hidden">
      ...
      <div className="containersec relative z-10 flex flex-col pb-7.5 px-4 sm:px-8 lg:px-16">
        ...
        <div className="w-full flex justify-between items-center mt-5">
          <div>
            <Link to={"/"}><img src="./logo.svg" alt="Logo" className="w-[140px] sm:w-32 cursor-pointer md:w-[240px] h-[21px]" /></Link>
          </div>
          <div className="flex gap-2 items-center">
            {!token && (
              <>
                <button onClick={() => { navigate("/login") }} className="bg-[#4681F4] h-10 w-[70px] sm:w-[98px] sm:h-[50px] cursor-pointer text-white rounded-full font-bold text-base sm:text-lg hover:bg-white hover:text-[#4681F4] border border-[#4681F4] transition-all">
                  Login
                </button>
                <button onClick={() => { navigate("/sign-up") }} className="border border-[#4681F4] cursor-pointer w-[90px] h-10 sm:w-[108px] sm:h-[50px] text-[#4681F4] rounded-full font-bold text-base sm:text-lg hover:bg-[#4681F4] hover:text-white transition-all">
                  Sign Up
                </button>
              </>
            )}
      ...
      <div className="w-full h-[150px] md:h-[200px] flex justify-center items-center  bg-[#4681F4] text-white relative overflow-hidden">
        ...
        <h3 className="text-2xl md:text-[32px] font-bold">Privacy Policy</h3>
